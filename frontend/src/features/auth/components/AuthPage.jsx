    ...
    <div className="min-h-screen bg-slate-900 px-4 py-10 text-slate-100">
      <div className="mx-auto w-full max-w-md rounded-2xl bg-slate-800 p-6 shadow-xl sm:p-8">
        ...
        <div className="mt-6 flex gap-2 rounded-lg bg-slate-700 p-1">
          <button
            className={`w-1/2 rounded-md px-3 py-2 text-sm ${mode === 'login' ? 'bg-slate-900' : ''}`}
            onClick={() => {
              setMode('login');
          ...
          <button
            className={`w-1/2 rounded-md px-3 py-2 text-sm ${mode === 'signup' ? 'bg-slate-900' : ''}`}
            onClick={() => {
              setMode('signup');
              dispatch(clearAuthError());
            }}
        ...
        {error && (
          <p className="mt-4 rounded-md bg-rose-500/20 p-2 text-sm text-rose-300">
            {error}
          </p>
        )}
        ...
        {mode === 'login' ? (
          <form className="mt-5 space-y-3" onSubmit={handleLogin}>
            ...
            />
            <button
              className="w-full rounded-md bg-indigo-500 px-3 py-2 font-medium text-white"
              disabled={loading}
            >
          ...
          <form className="mt-5 space-y-3" onSubmit={handleSignup}>
            ...
            <button
              className="w-full rounded-md bg-emerald-500 px-3 py-2 font-medium text-white"
              disabled={loading}
            >
